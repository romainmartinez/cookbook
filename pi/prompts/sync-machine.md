...
Read the relevant setup documentation and detect the operating system. Apply only the steps required by the pulled changes, such as creating symlinks, installing dependencies, or reloading configuration. Do not rerun the full setup, and remember that files behind existing symlinks update automatically. Ask before destructive actions, replacing unmanaged files, or commands requiring elevated privileges.

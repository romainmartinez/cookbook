...
Use one Question call with exactly three questions, ordered yesterday, today, then notes/blockers. Mention the target date in every question, set `multiple: true` on each, and follow these rules:

- Offer every evidence-backed candidate separately, strongest first. Mark the best default label `(Recommended)`.
- Keep labels to 1-5 words. Put the exact proposed standup line in the description unless an embedded mutation makes the final artifact uncertain; then show the proposed wording and state the uncertainty.
- Include `None` when an empty section is reasonable. Selecting `None` with another option selects the other option; use `- None` only when no substantive entry is selected.
- Use the automatic custom answer for additions, corrections, replacement wording, or missing work. Do not offer an all-or-nothing option.
...
Ask whether to copy a Teams-ready version, offering `Copy to clipboard (Recommended)` and `No`. If declined, stop.
